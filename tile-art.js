...
export function getTileBackSvgMarkup() {
  return [
    '<svg class="tile-image tile-art-svg tile-back-svg" viewBox="0 0 160 220" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">',
    `
      <rect x="12" y="16" width="136" height="192" rx="18" fill="#d5e9d1" />
      <rect x="6" y="6" width="148" height="200" rx="18" fill="#e7f5df" />
      <rect x="11" y="11" width="138" height="190" rx="15" fill="#dff0d6" opacity="0.98" />
    `,
    "</svg>",
  ].join("");
}
...
function renderTileFace(tileType) {
  ...
  } else if (/^f[1-8]$/.test(tileType)) {
    face = renderFlowerTile(tileType);
  } else {
...
function renderFlowerTile(tileType) {
  const label = FLOWER_LABELS[tileType] || tileType;
  const fill = FLOWER_COLORS[tileType] || COLORS.blue;

  return `
    ${renderFlowerBlossom(80, 55)}
    ${renderShadowedText(80, 144, label, fill, 84)}
  `;
}

function renderFlowerBlossom(cx, cy) {
  const petals = [
    [0, -11],
    [10, -3],
    [6, 10],
  ...
  return [
    ...petals.map(
      ([dx, dy]) =>
        `<circle cx="${formatNumber(cx + dx)}" cy="${formatNumber(cy + dy)}" r="6.2" fill="${COLORS.red}" opacity="0.86" />`,
    ),
    `<circle cx="${formatNumber(cx)}" cy="${formatNumber(cy)}" r="4.1" fill="#fff6d8" opacity="0.96" />`,
  ].join("");
}
